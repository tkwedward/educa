...
from .models import Job_detail, Test
...
def added(request):
    if request.method == "POST":
        ...
        if request.user.is_authenticated():
            if form.is_valid():
                form.save()
                user = User_data.objects.create(user=request.User, data_input='abs', number_of_count=1)
                test = Test.objects.create(title="hi")
                return redirect('success')
        else:
        ...
        form = ContactForm()
    return render(request, 'name.html', {'form': form, 'sam':sam})
